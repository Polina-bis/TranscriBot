...
async def start_circles(message: types.Message, state: FSMContext):
    # Инициализируем список для хранения ID сообщений
    await state.update_data(message_ids=[message.message_id])
    answer = await message.answer(circle_texts["start_circles"])
    ...
    await state.set_state(CircleStates.wait_circle)


@router.message(F.video_note, CircleStates.wait_circle)
async def download_circles(message: types.Message, state: FSMContext, bot: Bot):
    ...
    # скачиваем гс
    downloader = CircleDownloader(bot)
    file_path = await downloader.download_source("src/data/cash/circles", message.video_note.file_id)
    # сохраняем путь до гс
    await state.update_data({"circle_path": file_path})

    tokens = 1

    answer = await message.answer(
        text=circle_texts["choose_doing"],
        reply_markup=markup_choose_doing(tokens)
    )
    # Добавляем ID нового сообщения в state
    message_ids.append(answer.message_id)
    await state.update_data(message_ids=message_ids)
    await state.set_state(CircleStates.wait_doing)


@router.message(CircleStates.wait_circle)
async def download_circles(message: types.Message, state: FSMContext):
    ...
    answer = await message.answer(circle_texts["wrong_media"], reply_markup=markup.as_markup())
    # Добавляем ID нового сообщения в state
    message_ids.append(answer.message_id)
    await state.update_data(message_ids=message_ids)
    await state.set_state(CircleStates.wait_circle)
...
async def process_circles(callback: types.CallbackQuery, state: FSMContext):
    ...
    position = 3  # TODO: Реальная позиция в очереди
